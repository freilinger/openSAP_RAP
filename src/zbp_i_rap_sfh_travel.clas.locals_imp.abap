...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD calculateTotalPrice.
    ...
    MODIFY ENTITIES OF zi_rap_sfh_travel IN LOCAL MODE
        ENTITY travel
  ...
  METHOD calculateTravelID.
    ...
    READ ENTITIES OF zi_rap_sfh_travel IN LOCAL MODE
      ENTITY Travel
...
*    ENDIF.
    ...
    DATA business_data TYPE TABLE OF zrap_sfhz_travel_agency_es5.
    ...
    IF  agencies IS NOT INITIAL.
      ...
      TRY.
          ...
          NEW zcl_ce_rap_sfh_agency( )->get_agencies(
            EXPORTING
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( business_data[ agencyid = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_sfh_rap(
                                          ...
                                          textid   = zcm_sfh_rap=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_sfh_rap(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_sfh_rap=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_sfh_rap(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_sfh_rap=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_sfh_rap(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_sfh_rap=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_sfh_rap( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_sfh_rap=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
  ...
  METHOD is_create_granted.
    ...
    AUTHORITY-CHECK OBJECT 'ZSFH_TRST'
      ID 'ZSFH_TRAVS' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    ...
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZSFH_TRST'
        ID 'ZSFH_TRAVS' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZSFH_TRST'
        ID 'ZSFH_TRAVS' DUMMY
